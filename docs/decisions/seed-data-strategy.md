...
- 운영 R2에 이미 존재하는 메뉴 대표 이미지 메타데이터와 연결
...
3. `local`에서는 로컬 메뉴 이미지와 나머지 샘플 데이터를 같은 Runner에서 순서대로 초기화합니다.
...
- 메뉴 대표 이미지는 운영 R2에 이미 존재하는 자산만 로컬 seed 대상으로 등록합니다.
- 로컬 메뉴 이미지는 메뉴 code로 연결하고, 이미 이미지가 연결된 메뉴나 기존 이미지 자산 메타데이터를 덮어쓰지 않습니다.
- 운영 환경의 메뉴 이미지는 관리자 업로드 흐름으로만 생성합니다.
...
- 생성된 회원 ID는 기존 DB 상태에 따라 달라질 수 있으므로 로컬 테스트 토큰 발급 시 DB에서 확인한 `memberId`를 사용합니다.
...
- 로컬 메뉴 이미지 초기화를 두 번 실행해 중복이 없고 기존 메뉴 이미지가 보존되는지 확인합니다.
