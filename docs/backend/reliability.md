...
- 회원 물리 삭제 스케줄러는 삭제 예정 시각이 지난 회원을 작은 batch로 처리하고, 회원 한 명의 삭제 실패가 다른 회원의 삭제를 막지 않도록 분리합니다.
- 회원 물리 삭제 성공·실패는 민감정보 없이 `memberId`와 결과를 구조화 log로 남깁니다.
- 회원 물리 삭제는 자식 graph를 애플리케이션에 적재하지 않고 회원 행의 단일 `DELETE`와 DB FK cascade로 처리합니다.
