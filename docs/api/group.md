...
- 링크 관리 API와 링크 참여 API는 Bearer 인증이 필요한 보호 API입니다. 초대 토큰으로 그룹 정보를 미리 보는 API는 로그인 전에도 호출할 수 있습니다. 비회원의 로그인 유도와 로그인 후 참여 재시도는 클라이언트가 담당합니다.
...
- 현재 링크 조회는 만료된 링크를 반환하지 않으며 활성 링크가 없으면 `200`과 `data: null`을 반환합니다. 이 경우 클라이언트는 필요할 때 발급 API를 호출합니다.
- 초대받는 화면은 토큰으로 미리보기 API를 호출해 그룹명, 방장의 현재 닉네임, 활성 그룹원 수(방장 포함)를 표시합니다. 조회만으로 membership이나 링크 상태는 변경하지 않습니다. 그룹 ID, 초대 코드, 토큰 원문, 그룹원 목록은 반환하지 않습니다.
- 미리보기와 참여는 동일하게 토큰 존재·만료·그룹 `ACTIVE` 상태를 검사합니다. 토큰이 없으면 `404 GROUP_INVITE_LINK_NOT_FOUND`, 만료됐으면 `409 GROUP_INVITE_LINK_EXPIRED`, 그룹이 비활성이면 `409 GROUP_NOT_ACTIVE`입니다.
- backend access log에 토큰이 경로로 남지 않도록 미리보기와 참여 API는 `{"token":"..."}` request body로 토큰을 받습니다. 미리보기는 상태를 변경하지 않지만 토큰을 body에 담기 위해 POST를 사용합니다.
