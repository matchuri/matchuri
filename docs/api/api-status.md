...
- 중복 row와 backend drift는 `audit_api_contract.py`로 확인합니다.
...
- 중기 목표는 `api-status.yaml`을 SSOT로 두고 이 Markdown을 generated output으로 전환하는 것입니다.
- API ID, 상태, 기준 문서, 비고는 coordination metadata이므로 OpenAPI metadata만으로 대체하지 않습니다.
- method/path drift와 중복 row는 `audit_api_contract.py`로 검증합니다.
