...
- 삭제된 프리셋은 새로 선택할 수 없지만, 이미 해당 자산을 사용 중인 회원의 이미지는 유지합니다.
...
삭제는 프리셋 연결만 soft delete합니다. 기존 회원이 같은 자산을 사용 중일 수 있으므로 R2 객체와 `ImageAsset`은 삭제하지 않습니다.
...
서비스 사용자용 프리셋 목록 조회 API는 이번 요구사항에 포함하지 않습니다. 사용자 화면이 런타임에 선택지를 조회해야 한다면 별도 서비스 API 계약이 필요합니다.
